/*
 *Autor: José de Jesús García Piña
 *Correo: josegarciapina2001@gmail.com
 *Sept, 19, 2021
 */
#include <iostream>
#include <array>
...
{
    array<float, LEN> LlenarArreglo ={0}; //Creamos el arreglo y lo iniciamos en 0 como buena práctica
    for (int i = 0; i < LEN; ++i) { //Abarcamos todos los elementos
        cout << "Ingrese el valor del elemento " << i<< endl; //Asignamos valor a cada elemento
        cin >> LlenarArreglo[i];
    }
    return LlenarArreglo;
    }
...
array<float, LEN> MultiplicarArreglo(array<float, LEN> arreglo, float multiplo)
{
    for (int i = 0; i < arreglo.size(); ++i) {
        arreglo[i]= arreglo[i]*multiplo; //Realizamos la multiplicación de cada elemento por un escalar
    }
    return arreglo;
    }
...
array<float, LEN> SumarArreglos(array<float, LEN> arr1, array<float, LEN> arr2)
{
   array<float, LEN> SumarArreglos={0};
    for (int i = 0; i < LEN; ++i) {
        SumarArreglos[i]=arr1[i]+arr2[i];
    }
    return SumarArreglos;
}
