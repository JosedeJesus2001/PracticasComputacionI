...
void ImprimirSolucion(matriz & miMatriz)
{
    ...
    for (int i = 0; i < columnas; ++i) {
        cout << "x" << i << miMatriz [i][columnas]<<endl;
    }
}
    ...
    for(int i = 0; i < columnas; i++ ){
        ...
        if(miMatriz[i][i] == 0){
            ...
            for(int k = 0; k < columnas; k++){
                if (k != i){
                    piv = miMatriz[k][i];
                    for(int s = i; s < columnas + 1; s++){
                        miMatriz[k][s] = miMatriz[k][s] + piv * miMatriz[i][s] / miMatriz[i][i];
                    }
                }
                else{
                    piv = miMatriz[i][i];
                    for(int s = i; s < columnas + 1; s++){
                        miMatriz[k][s] = miMatriz[k][s] / piv;
                    }
                }
            }
        }
